...
export const filterScholarships = (
  ...
  return scholarships.filter((scholarship) => {
    ...
    if (
      ...
      const hasCategoryMatch = eligibility.categories.some(
        (c) => c.toLowerCase() === student.category.toLowerCase()
      );
      const isPwDOnly =
    ...
    if (
      ...
      !eligibility.gender.some(
        (g) => g.toLowerCase() === student.gender.toLowerCase()
      )
    ) {
      return false;
    }
    ...
    if (
      eligibility.courseLevels &&
      !eligibility.courseLevels.includes("ALL") &&
      !eligibility.courseLevels.some(
        (l) => l.toLowerCase() === student.courseLevel.toLowerCase()
      )
    ) {
      return false;
    }
    ...
    // 6. Course Type check
    if (
      eligibility.courseTypes &&
      !eligibility.courseTypes.includes("Any") &&
      !eligibility.courseTypes.some(
        (t) => t.toLowerCase() === student.courseType.toLowerCase()
      )
    ) {
      return false;
    }
